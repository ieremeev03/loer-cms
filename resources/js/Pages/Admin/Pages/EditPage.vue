                        ...
                        <template v-for="(item, index_i) in formItems.items"  :key="index_i">
                            <div class="p-4 bg-gray-50 rounded-lg space-y-2" :id="'item_'+index_i">
                                ...
                                <div class="flex flex-row items-center justify-end gap-2">
                                    ...
                                    <span v-if="index_i < Object.keys(formItems.items).length-1" class="cursor-pointer p-2 bg-indigo-500 rounded-lg text-xs text-white" @click="moveDown(index_i)">
                                        <svg fill="currentColor" height="10px" width="10px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                             viewBox="0 0 330 330" xml:space="preserve">
...
function addItem() {
    ...
    formItems.items[Object.keys(formItems.items).length] = cloneFields
}
...
function moveUp(index) {
    let t = formItems.items.splice(index,1);
    formItems.items.splice(index-1,0,t[0]);
}
